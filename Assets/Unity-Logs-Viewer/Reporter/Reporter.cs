...
public class Reporter : MonoBehaviour
{
    ...
    {
		this.openButtonClickCount++;
		if(this.openButtonClickCoroutine == null)
        {
			this.openButtonClickCoroutine = StartCoroutine(OpenButton_Co());
		}

		if(this.openButtonClickCount >= 3)
        {
			if (!show)
			{
				StopCoroutine(this.openButtonClickCoroutine);
				this.openButtonClickCoroutine = null;
				this.openButtonClickCount = 0;
				SetOpenButtonAlpha(0.0f);
				doShow();
			}
        }
	...
	private IEnumerator OpenButton_Co()
    {
		float maxTime = 1.0f;
		float accumTime = Time.deltaTime;
		SetOpenButtonAlpha(1.0f);

		while (true)
        {
			yield return new WaitForEndOfFrame();
			accumTime += Time.deltaTime;
			SetOpenButtonAlpha((maxTime - accumTime) / maxTime);

			if (accumTime >= maxTime)
            {
				openButtonClickCount = 0;
				SetOpenButtonAlpha(0.0f);
				break;
            }
        }
...
#elif ENABLE_INPUT_SYSTEM
		if (playerInputActions != null && playerInputActions.UI.Click.triggered == true)
		{
			downPos = playerInputActions.UI.Point.ReadValue<Vector2>();
			return downPos;
		}
#endif
		return Vector2.zero;
	...
	Vector2 getDrag()
	{
		...
		if (playerInputActions != null && playerInputActions.UI.Click.triggered == true)
		{
			return playerInputActions.UI.Point.ReadValue<Vector2>() - downPos;
		}
#endif
		return Vector2.zero;
	}
